...
export default function LikeCategory({ likecategory, userdata }) {
    ...
    return (
        <Fragment>
            ...
            <div className="flex justify-around items-center">
                {category.map((data, index) => (
                    <button href="#_"
                        ...
                        className="relative inline-flex items-center justify-center p-4 px-5 w-52 overflow-hidden font-medium 
                        text-indigo-600 transition duration-300 ease-out rounded-full shadow-xl 
                        group hover:ring-1 hover:ring-purple-500">
                        <span className={`absolute inset-0 w-full h-full ${showCategory == index ? ('bg-gradient-to-br from-sky-400 via-banner to-white') : ('bg-gradient-to-br from-sky-200 via-sky-200 to-pink-100')} `}></span>
                        <span className={`absolute bottom-0 right-0 block w-64 h-64 mb-32 mr-4 transition  duration-500 origin-bottom-left transform  rotate-45 translate-x-24 bg-sky-300 rounded-full opacity-30 group-hover:rotate-90 ease`}></span>
                        <span className={`relative   ${showCategory == index ? ('text-white') : ('text-black')}`}>{data}</span>
                    </button>
            ...
            <div className="grid grid-cols-6 gap-x-16 gap-y-9 py-10 mx-auto w-[90%] justify-center items-center">
                {showLikeCategory[showCategory].map((like, likedx) => (
                    <Fragment key={like.like_id}>
                        <LikeIcon
                            like={like}
                            hundleDeleteLike={hundleDeleteLike} hundleSetLike={hundleSetLike}
                            user_grade={userdata.user_grade} setTarget={setTarget} setViewModal={setViewModal} />
                    </Fragment>
                ))}
                <div className="grid justify-center items-center">
                    ...
                    >
                        <svg
                            className="hover:bg-gray-100 rounded-xl [&_path]:hover:fill-banner"
                            width="100"
                            height="100"
                            ...
                            <defs
                                id="defs1" />
                            <g
                                className="[&_path]:hover:fill-banner"
                                id="layer1">
                                <path
